<?php if ($activeTab === 'explore'): ?>
        ...
        <div class="openplay-card match-card-item" data-id="<?php echo $m['id']; ?>" data-level="<?php echo htmlspecialchars($m['level']); ?>" data-facility="<?php echo htmlspecialchars($searchData); ?>" data-location="<?php echo htmlspecialchars($m['location']); ?>">
          ...
          <div class="openplay-card-footer">
            ...
            <div class="openplay-actions-group">
              ...
              <?php else: ?>
                <button type="button" class="btn-openplay-join" id="btnJoinMatch_<?php echo $m['id']; ?>" onclick="joinOpenPlay('<?php echo $m['id']; ?>', '<?php echo htmlspecialchars(addslashes($facilityName)); ?>', '<?php echo htmlspecialchars(addslashes($m['type'] ?? 'Open Play')); ?>', '<?php echo htmlspecialchars(addslashes($m['date'] ?? '')); ?>', '<?php echo htmlspecialchars(addslashes($m['time'] ?? '')); ?>', <?php echo (float)($m['price'] ?? 200); ?>, '<?php echo htmlspecialchars(addslashes($m['level'] ?? 'All Levels')); ?>', '<?php echo htmlspecialchars(addslashes($m['location'] ?? '')); ?>')">
                  Join
                </button>
              <?php endif; ?>
            </div>
  ...
  <script>
    (function() {
      if (window._openplayRolloverTimer) clearInterval(window._openplayRolloverTimer);
      let lastCheckMinute = new Date().getMinutes();
      window._openplayRolloverTimer = setInterval(function() {
        const currentMinute = new Date().getMinutes();
        if (currentMinute !== lastCheckMinute) {
          lastCheckMinute = currentMinute;
          // Trigger smooth background refresh if user is inactive on Explore tab
          if (typeof filterMatchesLive === 'function') {
            filterMatchesLive();
          }
        }
      }, 30000);
